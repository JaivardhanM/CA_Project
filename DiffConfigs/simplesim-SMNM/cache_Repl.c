...
{ int i =0;
    printf("\n cp->name %s ; cp->tag_shift %d", cp->name, cp->tag_shift);
		...
		struct stat_sdb_t *sdb)	/* stats database */
{
  ...
  sprintf(buf, "%s.true_mnm_misses", name);
  stat_reg_counter(sdb, buf, "total number TRUE MNM of misses", &cp->true_mnm_misses, 0, NULL);
...
//Jai
void Calc_SMNM_HashValue(struct cache_t *cp, md_addr_t tag)
{
    int i, sum = 0;
    for (i = 1; i < (SMNM_SUM_WIDTH + 1); i++) 
    {
        if (tag & 0x1)
            sum += i * i;
        tag = tag >> 1;
    ...
    HashVal = sum;
    sum = 0;
    for (i = 1; i < (cp->tag_shift - SMNM_SUM_WIDTH + 1); i++) 
    {
        if (tag & 0x1)
            sum += i * i;
        tag = tag >> 1;
...
//rv
int mnm_check(struct cache_t *cp,md_addr_t tag)
{ int i=0;
  

  Calc_SMNM_HashValue(cp, tag);
  for(i=0;i<MNM_TABLE_SIZE;i++)
  {
    if((cp->mnm_table[i] == HashVal)&&(cp->mnm_table2[i] == HashVal2))
    {
      printf(" \n Returning MNM Check TRUE");
      return TRUE;
    }
...
//rv
int mnm_cursor = 0;
void mnm_update(struct cache_t *cp,md_addr_t tag,int type)
{
  int i=0 ;
  //int HashVal = 0;
  ...
  if(type == PLACE)
  {
    if (!mnm_check(cp, tag))
    {
        for(i=0;i<MNM_TABLE_SIZE;i++)
        {
          if((cp->mnm_table[i] == 0) && (cp->mnm_table2[i] == 0))
          {
            printf("\n Writing Hash Values HashVal %d HashVal2 %d", HashVal, HashVal2);
            cp->mnm_table[i] = HashVal;
            cp->mnm_table2[i] = HashVal2;
            break;
          }
          ...
          if(i == MNM_TABLE_SIZE)
          {
            ...
            printf( " \n printing Table \n");            
            for (i = 0; i<MNM_TABLE_SIZE;i++)
                printf(" %d ", cp->mnm_table[i]);
            printf( " \n printing Table2 \n");     
            for (i = 0; i<MNM_TABLE_SIZE;i++)
                printf(" %d ", cp->mnm_table[i]);
          }
   
        /*cp->mnm_table[mnm_cursor] = HashVal;
        mnm_cursor++;
        mnm_cursor = mnm_cursor % MNM_TABLE_SIZE;

        printf(" \n Cache level : %s mnm_cursor: %d ", cp->name, mnm_cursor);
        if ( mnm_cursor == 0)
        {
            printf( " \n printing Table \n");            
            for (i = 0; i<MNM_TABLE_SIZE;i++)
  ...
  if(type == REPL)
  {
      for(i=0;i<MNM_TABLE_SIZE;i++)
      {
        if((cp->mnm_table[i] == HashVal) && (cp->mnm_table2[i] == HashVal2))
        {
            /*while(i < MNM_TABLE_SIZE)
            {
                cp->mnm_table[i] = cp->mnm_table[i+1];
                i++;
            }*/
            cp->mnm_table[i] = 0;
            cp->mnm_table2[i] = 0;
