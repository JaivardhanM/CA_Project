...
//Jai
int Calc_SMNM_HashValue(struct cache_t *cp, md_addr_t tag)
{
    int i, sum = 0;
    for (i = 1; i < (cp->tag_shift +1)/*(SMNM_SUM_WIDTH + 1)*/; i++) 
    {
        if (tag & 0x1)
            sum += i * i;
        tag = tag >> 1;
...
{ int i=0;
  ...
  HashVal = Calc_SMNM_HashValue(cp, tag);
  for(i=0;i<MNM_TABLE_SIZE;i++)
  {
    if(cp->mnm_table[i] == HashVal)
    {
      return TRUE;
    }
...
{
  int i=0, j=0;
  int HashVal = 0, Index = 0, Val_Present;
  ...
  if(type == PLACE)
  {
    if (!Val_Present)
    {
      ...
      if( mnm_cursor != MNM_TABLE_SIZE)
      {
        mnm_cursor++;
      }
      for(j = mnm_cursor; j>0 ; j--)
      {
        cp->mnm_table[j] = cp->mnm_table[j-1];
      }
    }
    else
    {
      HashVal = cp->mnm_table[Index];
      for(j = Index; j>0 ; j--)
      {
        cp->mnm_table[j] = cp->mnm_table[j-1];
      }
      cp->mnm_table[0] =  HashVal;
    }
