...
//tournament length
static int tournament_length;
static int hits2win;

static int accesses_bw_tournaments;
	      ...
	      tick_t now)		/* time of access */
{
  ...
  if (cache_dl3)
  ...
  else
    {
      ...
      if (cmd == Read)
      {
		    int lat1 = mem_access_latency(bsize);
        //printf("\n dl2 mem access lat: %d \n",lat1);
	      return lat1;
	      }
      else
		{
...
/* l2 inst cache l1 block miss handler function */
static unsigned int			/* latency of block access */
		  ...
		  int argc, char **argv)        /* command line arguments */
{
  ...
  if (!mystricmp(cache_dl1_opt, "none"))
	...
	fatal("bad l1 D-cache parms: <name>:<nsets>:<bsize>:<assoc>:<repl>");
      cache_dl1 = cache_create(name, nsets, bsize, /* balloc */FALSE,
			       ...
			       dl1_access_fn, /* hit lat */cache_dl1_lat,0,0,0,0);

		// rv // init MNM cache as false
		cache_dl1->isMNM =FALSE;
      ...
      if (!mystricmp(cache_dl2_opt, "none"))
	  {
		...
		if (strcmp(cache_dl3_opt, "none")) //rv
      ...
      else
	{
	  ...
	  cache_dl2 = cache_create(name, nsets, bsize, /* balloc */FALSE,
				   /* usize */0, assoc, cache_char2policy(c),
				   dl2_access_fn, /* hit lat */cache_dl2_lat,0,0,0,0);
	  //rv
	  cache_dl2->isMNM = TRUE;
      ...
      else
	{
	  ...
	  cache_dl3 = cache_create(name, nsets, bsize, /* balloc */FALSE,
				   /* usize */0, assoc, cache_char2policy(c),
				   dl3_access_fn, /* hit lat */cache_dl3_lat,0,0,0,0);
	  //rv
	  cache_dl3->isMNM = TRUE;
	}
  ...
  if (!mystricmp(cache_il1_opt, "none"))
	...
	fatal("the l1 & l2 inst cache must defined if the l3 cache is defined");
      cache_il3 = NULL;
    }
    ...
    {
      if (sscanf(cache_il1_opt, "%[^:]:%d:%d:%d:%c:%d:%d:%d:%d",
          name, &nsets, &bsize, &assoc, &c,
          &miss_sat,&tournament_length,&accesses_bw_tournaments,&hits2win) != 9)
            fatal("bad l1 I-cache params: <name>:<nsets>:<bsize>:t:<repl>:<miss_sat>:<tournament_length>:<accesses_bw_tournaments>:<hits2win>");
      ...
      else
      {
        cache_il1 = cache_create(name, nsets, bsize, /* balloc */FALSE,
         /* usize */0, assoc, cache_char2policy(c),
         il1_access_fn, /* hit lat */cache_il1_lat,0,0,0,0);
      }

		//rv
	  cache_il1->isMNM = FALSE;
      ...
      else
	{
	  ...
	  cache_il2 = cache_create(name, nsets, bsize, /* balloc */FALSE,
				   /* usize */0, assoc, cache_char2policy(c),
				   il2_access_fn, /* hit lat */cache_il2_lat,0,0,0,0);
	  //rv
	  cache_il2->isMNM = TRUE;
	  // check l3 opts
      ...
      else
	{
	  ...
	  cache_il3 = cache_create(name, nsets, bsize, /* balloc */FALSE,
				   /* usize */0, assoc, cache_char2policy(c),
				   il3_access_fn, /* hit lat */cache_il3_lat,0,0,0,0);
	  //rv
	  cache_il3->isMNM = TRUE;
...
sim_reg_stats(struct stat_sdb_t *sdb)   /* stats database */
{
  ...
  if (cache_il3
      && (cache_il3 != cache_dl1 && cache_il3 != cache_dl2 && cache_il3 != cache_dl3))
		  ...
		  if (cache_dl1)
		    {
		      ...
		      lat =
			cache_l1_access(cache_dl1, Write, (LSQ[LSQ_head].addr&~3),
		  ...
		  if (dtlb)
		    {
		      /* access the D-TLB */
		      lat =
			cache_l1_access(dtlb, Read, (LSQ[LSQ_head].addr & ~3),
				     NULL, 4, sim_cycle, NULL, NULL);
		      if (lat > 1)
			events |= PEV_TLBMISS;
		    }
			      ...
			      == (F_MEM|F_LOAD)))
			{
			  ...
			  if (!load_lat)
			      ...
			      if (cache_dl1 && valid_addr)
				{
				  ...
				  load_lat =
				    cache_l1_access(cache_dl1, Read,
						 (rs->addr & ~3), NULL, 4,
						 sim_cycle, NULL, NULL);
			  ...
			  if (dtlb && MD_VALID_ADDR(rs->addr))
			    {
				 ...
				 initiate speculative TLB misses */
			      tlb_lat =
				cache_l1_access(dtlb, Read, (rs->addr & ~3),
					     NULL, 4, sim_cycle, NULL, NULL);
			      if (tlb_lat > 1)
				events |= PEV_TLBMISS;
	  ...
	  if (cache_il1)
	    {
	      /* access the I-cache */
	      lat =
		cache_l1_access(cache_il1, Read, IACOMPRESS(fetch_regs_PC),
	  ...
	  if (itlb)
	    {
		 ...
		 speculative TLB misses */
	      tlb_lat =
		cache_l1_access(itlb, Read, IACOMPRESS(fetch_regs_PC),
			     NULL, ISCOMPRESS(sizeof(md_inst_t)), sim_cycle,
			     NULL, NULL);
	      if (tlb_lat > 1)
